"""Depletion manager for coupled transport + depletion simulations.

This module provides :class:`DepletionManager`, a single entry point for
running depletion simulations.  Heavy lifting (transport dispatch, rate
extraction, normalization, scheme interpretation, and CRAM solves) is
...
from .integration_schemes import SCHEMES, FALLBACK_SCHEMES
...
class DepletionManager:
    ...
    prev_results : str, Path, or openmc.deplete.Results, optional
        Previous depletion results to restart from.  Can be a file
        path to ``depletion_results.h5`` or a :class:`Results` object.
        When provided, initial compositions are loaded from the last
        entry and new steps are appended.  If the number of provided
        timesteps exceeds the number of completed steps **and** the
        leading values match, the completed prefix is automatically
        skipped ("resubmit same script" mode).  Otherwise all provided
        timesteps are treated as new steps to append.
    ...
    def __init__(
        ...
        *,
        ...
        prev_results: str | Path | None = None,
    ):
        # --- Validate inputs ---
        ...
        # Fallback scheme name (LE/QI → lower-order on step 0)
        self._scheme_fallback_name = FALLBACK_SCHEMES.get(self._scheme_name)
        ...
        # Handle prev_results for restart
        from .results import Results as _Results
        if prev_results is not None:
            if isinstance(prev_results, (str, Path)):
                self._prev_results_path = Path(prev_results)
                self._prev_results = _Results(
                    str(self._prev_results_path))
            elif isinstance(prev_results, _Results):
                self._prev_results_path = None
                self._prev_results = prev_results
            else:
                raise TypeError(
                    f"prev_results must be a file path or Results "
                    f"object, got {type(prev_results).__name__}")
            if len(self._prev_results) == 0:
                raise ValueError(
                    "prev_results contains no depletion steps.")
        else:
    ...
    def _initialize(self):
        ...
        # If restarting, transfer volumes from previous results so that
        # the model materials carry the same volumes used in the prior run.
        if self._prev_results is not None:
            self._prev_results[-1].transfer_volumes(self._model)
    ...
    def _get_compositions_from_results(self):
        """Load atom counts from the last entry of previous results.
        ...
        """
        last = self._prev_results[-1]
        nuclide_dict = self._chain.nuclide_dict

        result = []
        for mat_id in self._burn_mat_ids:
            n = np.zeros(self._n_chain)
            if mat_id not in last.index_mat:
                raise ValueError(
                    f"Depletable material {mat_id} not found in "
                    "previous results. Ensure the same model is "
                    "used for restart.")
            mat_idx = last.index_mat[mat_id]
            for nuc_name, nuc_idx in last.index_nuc.items():
                if nuc_name in nuclide_dict:
                    n[nuclide_dict[nuc_name]] = last.data[mat_idx, nuc_idx]
            result.append(n)
        return result

    def _setup_restart(self):
        """Prepare simulation state from previous results.

        If the total number of provided timesteps exceeds the number of
        previously completed steps and the leading timesteps/source-rates
        match, the completed prefix is automatically skipped
        (auto-continue mode).  Otherwise all provided timesteps are
        treated as new steps to append.

        Returns
        -------
        compositions : list of numpy.ndarray
            BOS atom counts loaded from previous results.
        start_time : float
            Simulation time [s] at which to resume.
        step_offset : int
            HDF5 step index for the first new result entry.

        """
        prev_res = self._prev_results

        # Detect whether last entry is a zero-width "final" entry
        has_final = (prev_res[-1].time[0] == prev_res[-1].time[1])
        n_prev = len(prev_res) - 1 if has_final else len(prev_res)

        if n_prev == 0:
            raise ValueError(
                "Previous results contain no completed steps.")

        # Extract previous timesteps and source rates directly from
        # entries (avoids dependence on Results helper methods that
        # assume a final entry exists).
        prev_dt_s = np.array([
            prev_res[i].time[1] - prev_res[i].time[0]
            for i in range(n_prev)
        ])
        prev_sr = np.array([
            prev_res[i].source_rate for i in range(n_prev)
        ])

        n_total = len(self._timesteps_s)

        # Auto-continue: if the user provides at least as many timesteps
        # as previously completed, and the leading values match, skip
        # the already-completed prefix.
        if n_total >= n_prev:
            prefix_dt = self._timesteps_s[:n_prev]
            prefix_sr = self._source_rates[:n_prev]
            if (np.allclose(prefix_dt, prev_dt_s, rtol=1e-12) and
                    np.allclose(prefix_sr, prev_sr, rtol=1e-12)):
                self._timesteps_s = self._timesteps_s[n_prev:]
                self._source_rates = self._source_rates[n_prev:]
                self._transport_mask = self._transport_mask[n_prev:]
            elif n_total > n_prev:
                raise ValueError(
                    "When providing more timesteps than previously "
                    "completed, the leading timesteps and source "
                    "rates must match the previous results.  To "
                    "append new timesteps, provide only the new ones.")

        # Load compositions from last entry
        compositions = self._get_compositions_from_results()
        start_time = float(prev_res[-1].time[1])

        return compositions, start_time, n_prev

    def _save_final_step(self, step_idx, n_list, t, source_rate, k_eff):
        """Write the final zero-width entry for Results API compatibility.

        The :class:`Results` querying methods (``get_times``,
        ``get_source_rates``) expect the results file to end with an
        extra entry whose start and end times are identical.
        """
        res = StepResult()
        res.k = (k_eff, 0.0)
        res.time = [t, t]
        ...
        res.allocate(
            self._volumes, nuc_list, burn_list, burn_list,
            name_list=self._burn_mat_names,
        )

        for i in range(len(self._burn_mat_ids)):
            for j in range(len(nuc_list)):
                res.data[i, j] = n_list[i][j]

        rates = ReactionRates(burn_list, nuc_list, self._reactions)
        res.rates = rates
    ...
    def run(self):
        ...
        try:
            ...
            if self._prev_results is not None:
                n, t, step_offset = self._setup_restart()
                n_steps = len(self._timesteps_s)
                if n_steps == 0:
                    warnings.warn(
                        "All timesteps already completed in previous "
                        "results. No new steps to run.",
                        stacklevel=2,
                    )
                    return self._prev_results
                # Ensure output file exists for appending
                if not self._output_path.exists():
                    if self._prev_results_path is not None:
                        self._output_path.parent.mkdir(
                            parents=True, exist_ok=True)
                        shutil.copy2(str(self._prev_results_path),
                                     str(self._output_path))
                    else:
                        raise ValueError(
                            "Output file does not exist and "
                            "prev_results was not given as a file "
                            "path.  Use the same output_path as the "
                            "previous run, or provide prev_results "
                            "as a file path.")
            else:
            ...
            for i, (dt, source_rate) in enumerate(
                ...
                self._save_step(step_offset + i, n, n_end, t, dt,
                                source_rate, k_eff, proc_time)
            ...
            self._save_final_step(
                step_offset + len(self._timesteps_s), n, t,
                source_rate, k_eff)
